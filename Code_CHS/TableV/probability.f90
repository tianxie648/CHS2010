...
! Contains LN_PDF: LN_Normal,LN_Gamma,LN_Exponential,LN_Dirichlet (for when working with logs of pdfs)
...
REAL(8), PARAMETER  :: zigm1=2147483648.0d0,zigm2=2147483648.0d0,half=0.5d0,small=1.0D-250,big=1.0D250
...
REAL(8) FUNCTION MULTINORMPDF(n,z,mu,varin)
IMPLICIT NONE
INTEGER, INTENT(IN) :: n
REAL(8), INTENT(IN) :: z(n)
REAL(8), INTENT(IN) :: mu(n),varin(n,n)
REAL(8), PARAMETER :: logroot2pi = 0.918938533204672780563271317078D0
REAL(8) :: arg(n),scalar,var(n,n),invvar(n,n),dett,sqrtdett
var=varin
dett = det(varin)
invvar = matinv(var)
scalar = -0.5d0*dot_product((z-mu),matmul(invvar,(z-mu)))
scalar = -dble(n)*logroot2pi - 0.5*DLOG(dett) + scalar
multinormpdf = DEXP(scalar)
END FUNCTION MULTINORMPDF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! LOG OF PDFS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
REAL(8) FUNCTION LN_NORMPDF(z,mu,varin)
IMPLICIT NONE
REAL(8), INTENT(IN) :: z
REAL(8), INTENT(IN) :: mu,varin
REAL(8), PARAMETER :: logroot2pi = 0.918938533204672780563271317078D0
...
IF (DSQRT(var)<small) var=small
IF (var<0.0d0) THEN
	PAUSE '***FATAL ERROR: variance has to be positive (LN_NORMPDF)***'
    STOP
END IF
arg = -0.5D0*(z-mu)*(z-mu)/var
ln_normpdf = -logroot2pi - 0.5*DLOG(var) + arg
END FUNCTION LN_NORMPDF

REAL(8) FUNCTION LN_GAMPDF(x,a,b)
! Returns the pdf of a gamma distribution evaluated at x. a is shape parameter and b inverse scale.
! such that mean=a/b and var=a/(b*b)
! That is P(x) = [(b^a)/Gamma(a)] * [x^(a-1)] * exp(-x*b) for x>0 and a>=1.
IMPLICIT NONE
REAL(8), INTENT(IN) :: x,a,b
IF (a<0.0d0) THEN
    PAUSE '***FATAL ERROR: a has to be positive (LN_GAMPDF)***'
    STOP
END IF
IF (b<0.0d0) THEN
	PAUSE '***FATAL ERROR: b has to be positive (LN_GAMPDF)***'
    STOP
END IF
IF (x>0) THEN
    LN_GAMPDF=(a-1.0d0)*DLOG(x)-(x*b)-gammln(a)+a*DLOG(b)
ELSE IF ((x==0.0d0).AND.(a<1.0d0)) THEN
    LN_GAMPDF=DLOG(big)
ELSE IF ((x==0.0d0).AND.(a==1.0d0)) THEN
    LN_GAMPDF=DLOG(b)
ELSE IF ((x==0.0d0).AND.(a>1.0d0)) THEN
	LN_GAMPDF=-big
END IF
END FUNCTION LN_GAMPDF

REAL(8) FUNCTION LN_EXPPDF(x,a)
! PDF OF EXPONENTIAL DISTRIBUTION P(x|a) = (1/a)*exp(-x/a)
IMPLICIT NONE
REAL(8), INTENT(IN) :: x,a
IF (a<0.0d0) THEN
    PAUSE '***FATAL ERROR: a has to be positive (LN_EXPPDF)***'
    STOP
END IF
LN_EXPPDF=(-x/a) - DLOG(a)
END FUNCTION LN_EXPPDF

REAL(8) FUNCTION LN_DIRICHPDF(x,a)
IMPLICIT NONE
...
aux=gammln(SUM(a))
DO j = 1, SIZE(a)
    aux = aux - gammln(a(j)) + (a(j)-1.0d0)*DLOG(x(j))
END DO
LN_DIRICHPDF = aux
END FUNCTION LN_DIRICHPDF
...
REAL(8), PARAMETER :: EPS=4.0d0*2.220446049250313D-016
INTEGER :: n
REAL(8) :: ap,del,summ
if (x == 0.0d0) then
	gser=0.0d0
...
REAL(8), OPTIONAL, INTENT(OUT) :: gln
INTEGER, PARAMETER :: ITMAX=1000
REAL(8), PARAMETER :: EPS=2.220446049250313D-016,FPMIN=1.0D-250/EPS
INTEGER :: i
