...
log.likelihood <- function(gamma,phi,delta.eta,Z,a0,P0,...){
  ...
  # -------------------------------------------------------------- #
  # Initial conditions
  moments <- UT2(N.theta,a0,P0,k=2,f="CES",h="linear",delta.eta=delta.eta,gamma=gamma,phi=phi)
  ...
  for (t in 1:T.sample) {
    ...
    moments <- UT2(N.theta,aa[t,],PP[t,,],k=2,f="CES",h="linear",delta.eta=delta.eta,gamma=gamma,phi=phi)
