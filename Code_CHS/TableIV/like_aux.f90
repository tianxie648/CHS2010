...
real(8) function density(i)
    ...
    do m = 1, nemf
        do j = 1, nanch
            aANCH(m,j,:)   = a(t,m,nobs(t)+1,:)
            PANCH(m,j,:,:) = P(t,m,nobs(t)+1,:,:)
            WANCH(j,m)     = W(t,m,nobs(t)+1)
        end do            
    end do        
    ...
    do j = 1, nanch
        if (dYANCH(i,j) == 0) then
            do m = 1, nemf
                aANCH(m,j+1,:)   = aANCH(m,j,:)
                PANCH(m,j+1,:,:) = PANCH(m,j,:,:)
                WANCH(j+1,m)     = WANCH(j,m)
            end do
            ...
            if (PROBANCH(j) == 0) then
                ...
                do m = 1, nemf
                    ...
                    do k1 = 1, nsigma
                        ...
                        KFANCH(m,j,:) = KFANCH(m,j,:) + wsigma(k1)*(YSTAR-muanch(j,m))*(asigma(k1,:)-aanch(m,j,:))  
                    end do
		            vanch(m,j) = YANCH(i,j) - muanch(j,m)
		            aanch(m,j+1,:)  = aanch(m,j,:) + KFANCH(m,j,:)*vanch(m,j)/varanch(j,m)
		            PANCH(m,j+1,:,:) = PANCH(m,j,:,:) - outerprod(KFANCH(m,j,:),KFANCH(m,j,:))/varanch(j,m)
