...
subroutine UT(n,m,a,k,P,asigma,wsigma)
    ...
    if (iP0 == 0) then
    !   The option below calculates the square root matrix.  You may not want to do it here for three reasons: 
...
real(8) function transition(s,ifacc,nfacc,a)
    ...
    if (ifacc <= 1) then
        ...
        temp = alpha(1,ifacc)*phi(s,ifacc)
        transition = const(s,ifacc)/alpha(1,ifacc) + (rho(s,ifacc)/temp)*dlog(transition)
    end if
